...
export default function Facebook() {
  ...
  const downloadVideo = async () => {
    ...
    axios.post("https://apiv3.uncoverecom.com/download-fb", {
      videoUrl: videoLink,
    }).then((response) => {
      var downloadUrl = response.data.data[0].url;
      var random = Math.floor(Math.random() * 100000);
      handleDownload(downloadUrl, `downloaded-video${random}-Bandoffads.mp4`);
      setIsLoading(false);
    }).catch((error) => {
    ...
    <>
      <div className="w-full h-full flex flex-col gap-5 items-center justify-center">
        <div className="flex flex-col gap-5 absolute w-full">
          <h1 className="text-center text-white text-4xl font-semibold">Facebook Video Downloader</h1>
